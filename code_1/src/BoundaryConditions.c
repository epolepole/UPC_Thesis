...
void InletBC(CellProps *Cells, int i)
{
    ...
    if ((Cells + i)->Boundary == 2) // INLET BOUNDARY
    {
        if ((Cells + i)->Corner == 1) // CORNER
        {
            ...
            // THEORETICALLY, THIS CONDITION SHOULD ALWAYS BE FALSE, BECAUSE ALL CORNERS
            // HAVE Boundary = 1 BY DEFINITION (SEE CORNER DETERMINATION IN CellFunctions.c)
        }
        ...
        else // NOT A CORNER
        {
            if ((Cells + i)->CoordZ == 0) // INLET PLANE xy, z=0
            {
                ...
                (Cells + i)->F[15] = (Cells + i)->F[18] + Rho / 6 * (W + U) - N2;
                (Cells + i)->F[16] = (Cells + i)->F[17] + Rho / 6 * (W - U) + N2;
            }
            ...
            else if ((Cells + i)->CoordY == 0) // INLET PLANE xz, y=0
            {
                ...
                (Cells + i)->F[3] = (Cells + i)->F[4] - (Rho * U) / 3;
            ...
            else if ((Cells + i)->CoordY == YMAX) // INLET PLANE xz, y=ymax
            {
                ...
                (Cells + i)->F[4] = (Cells + i)->F[3] - (Rho * U) / 3;
...
void WallBC(CellProps *Cells, int i, int* opp)
{
    ...
    U = (Cells + i)->U; // |                             U = (Cells + i)->Uo;
    V = (Cells + i)->V; // |---> Maybe I have to write:  V = (Cells + i)->Vo;
    W = (Cells + i)->W; // |                             W = (Cells + i)->Wo;

    if ((Cells + i)->Boundary == 1) // WALL BOUNDARY
    {
        /*if ((Cells + i)->Corner == 1) // CORNER -> WALL & INLET/OUTLET IN THE SAME NODE
        {
            printf("Corner is still being used somewhere. See WallBC in BoundaryConditions.c\n");
        }
